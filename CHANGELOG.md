...
### Added
- `services.non_blocking` flag for command steps. When `true` and the Docker daemon is not running, the services dependency (compose + containers) is skipped with a warning and the command continues instead of failing. Defaults to `false` (unchanged behaviour). Daemon-availability is detected via `docker info`; if the daemon is running, a genuine compose/container failure still blocks as before.
