...
func TestParseCompletionContext(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "completing command name with alias",
			commandLine: "dt te",
			projectDir:  "/test",
			expectedContext: &CompletionContext{
				Words:       []string{"te"},
		...
		{
			name:        "completing flag with alias",
			commandLine: "dt --ver",
			projectDir:  "/test",
			expectedContext: &CompletionContext{
				Words:       []string{"--ver"},
...
func TestGetAllAvailableCommands(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedCommands: []string{
				"cleanup-all", "cleanup-pids", "completion", "logs", "onboard", "restart", "schema", "status", "stop", "validate", "version",
			},
		},
		{
			...
			expectedCommands: []string{
				"build", "cleanup-all", "cleanup-pids", "completion", "deploy", "logs", "onboard", "restart", "schema", "status", "stop", "test", "validate", "version",
			},
		},
	}
